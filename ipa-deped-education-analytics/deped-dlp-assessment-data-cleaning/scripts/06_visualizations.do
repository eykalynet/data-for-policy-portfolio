...
graph hbar student_count, over(metric_order, label(labsize(small))) over(period_order, label(labsize(small))) asyvars ///
    ...
    subtitle("Beginning and End of School Year Counts Across Randomized DLP Schools") ///
...
replace category_order = 2 if reading_category == "frustration"
replace category_order = 3 if reading_category == "instructional"
label define category_order 1 "Grade Ready" 2 "Frustration" 3 "Instructional", replace
...
foreach bosy_group in "2-Level" "3-Level" {
    ...
    graph bar percent_of_classified, over(category_order, label(angle(30) labsize(vsmall))) ///
        over(grade_order, label(labsize(small))) asyvars ///
        by(comparison_order, cols(2) title("Philippine Informal Reading Inventory Reading Categories") subtitle("Beginning of School Year `bosy_group' Grouping and End of School Year Proxy") note("End of School Year independent is used as a proxy for grade ready because End of School Year data do not include the Beginning of School Year 2-level and 3-level grade-ready fields.", size(*.65))) ///
        subtitle(, nobox fcolor(none) lcolor(none)) ///
...
graph bar compliance_rate, over(assignment_group_label, label(angle(30) labsize(small))) asyvars ///
    title("Compliance Rates by Assigned Randomization Group") ///
    ...
    note("Compliance is based on assignment group and observed treatment status.", size(*.7) span)
