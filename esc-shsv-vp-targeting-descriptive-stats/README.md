# ESC-SHSV-VP Targeting Descriptive Statistics
...
The dummy generator reads the expected form labels from `scripts/02_hfc_checks.R`, writes `raw/dummy_esc_shsv_vp_targeting_responses.xlsx`, and includes a few intentional issues: an invalid email, an invalid LRN, a household-size inconsistency, an unexpected yes/no value, and one under-18 respondent without age confirmation. The master script reads this dummy workbook by default. Replace `config$input_file` in `scripts/00_master.R` when the real export is ready.

## Main Outputs

```text
data/esc_shsv_vp_targeting_clean_data.csv
data/esc_shsv_vp_targeting_school_ppi.csv
outputs/esc_shsv_vp_targeting_issue_log.csv
outputs/esc_shsv_vp_targeting_summary.csv
```
